...
public class CancionesWebController {
    ...
    private ResponseEntity<List<Cancion>> getCancionByGeneroRock(){
        return new ResponseEntity<>(service.getCancionByGenero(4), HttpStatus.OK);   
    }
    
    @GetMapping("/api/catpop")
    private ResponseEntity<List<Cancion>> getCancionByGeneroPop(){
        return new ResponseEntity<>(service.getCancionByGenero(1), HttpStatus.OK);
    }
    
    @GetMapping("/api/catelec")
    private ResponseEntity<List<Cancion>> getCancionByGeneroElect(){
        return new ResponseEntity<>(service.getCancionByGenero(2), HttpStatus.OK);
    }
    
    @GetMapping("/api/catragg")
    private ResponseEntity<List<Cancion>> getCancionByGeneroRagg(){
        return new ResponseEntity<>(service.getCancionByGenero(3), HttpStatus.OK);
    }
}
